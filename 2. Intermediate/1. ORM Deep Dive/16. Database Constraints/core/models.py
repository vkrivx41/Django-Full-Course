    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                violation_error_message="Latitude needs to be in the range [-90, 90]"
            ),
            models.CheckConstraint(
                name="longitude_constraint",
                condition=Q(longitude__gte=-180, longitude__lte=180),
                violation_error_message="Longitude needs to be in the range [-180, 180]"
            ),
            models.UniqueConstraint(
                name='name_website_unique',
                fields=['name', 'website'],
                violation_error_message="Name and website can't match with another restaurant"
            )
        ]
...
class Order(models.Model):
    product = models.ForeignKey(to=Product, on_delete=models.CASCADE, related_name="orders_in")
