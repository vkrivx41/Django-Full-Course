...
class PresenceConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if message_type == "message":
            # the method to send the message, replace "message"
            json_data["type"] = "send_message_to_receiver"

            if not json_data['content'] and not json_data['attachment']:
                return
    ...
    async def send_message_to_receiver(self, event):
        event['type'] = 'chat'  # for js to understand
